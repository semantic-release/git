 ...
 *
 * @param {String} origin The remote repository URL.
 * @param {String} targetBranch The branch to push.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @throws {Error} if the push failed.
 */
async function pushBranch(origin, targetBranch, execaOpts) {
  await execa('git', ['push', '--tags', '--set-upstream', origin, `HEAD:${targetBranch}`], execaOpts);
}
...
module.exports = {getModifiedFiles, add, gitHead, commit, push, pushBranch};
