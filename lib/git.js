...
async function filterModifiedFiles(files, execaOpts) {
  const deletedFiles = (await execa('git', ['ls-files', '-d'], execaOpts)).stdout;
  const filteredModifiedFiles =
    files.length > 0 ? (await execa('git', ['ls-files', '-m', '-o', ...files], execaOpts)).stdout : '';

  return [deletedFiles, filteredModifiedFiles]
    .join('\n')
    .split('\n')
    .map(file => file.trim())
