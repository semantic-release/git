const {isPlainObject, isArray, isNull, template, castArray, uniq} = require('lodash');
const micromatch = require('micromatch');
const dirGlob = require('dir-glob');
const pReduce = require('p-reduce');
const debug = require('debug')('semantic-release:git');
const resolveConfig = require('./resolve-config');
const {getModifiedFiles, add, commit, push, pushBranch} = require('./git');
...
module.exports = async (pluginConfig, context) => {
  ...
  const {message, assets, targetBranch} = resolveConfig(pluginConfig, logger);
  ...
  if (filesToCommit.length > 0) {
    ...
    if (isNull(targetBranch)) {
      await push(repositoryUrl, branch.name, {env, cwd});
    } else {
      await pushBranch(repositoryUrl, targetBranch, {env, cwd});
    }
