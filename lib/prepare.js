 ...
 * @param {Boolean} [pluginConfig.push_remote] Whether to push to the remote repository.
...
export default async function prepare(pluginConfig, context) {
  ...
  const { message, assets, push_remote } = resolveConfig(pluginConfig, logger);
  ...
  const filesToCommit = uniq(
    await pReduce(
      assets.map((asset) =>
        !isArray(asset) && isPlainObject(asset) ? asset.path : asset,
      ),
      async (result, asset) => {
        ...
        return [
          ...result,
          ...micromatch(modifiedFiles, await dirGlob(glob, { cwd }), {
            dot: true,
            nonegate,
            cwd,
            expand: true,
  ...
  if (filesToCommit.length > 0) {
    ...
    if (push_remote) {
      ...
      logger.log("Pushed Git release %s to remote", nextRelease.gitTag);
    }
  }
}
