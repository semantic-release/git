...
module.exports = async (pluginConfig, {options: {branch, repositoryUrl}, lastRelease, nextRelease, logger}) => {
	const resolvedConfig = resolveConfig({branch, ...pluginConfig}, logger);
	const patterns = [];
	const modifiedFiles = await getModifiedFiles();

	patterns.push(
		...resolvedConfig.assets.map(pattern => (!isArray(pattern) && isPlainObject(pattern) ? pattern.path : pattern))
	);
	...
	if (filesToCommit.length > 0) {
		...
		await commit(template(resolvedConfig.message)({branch, lastRelease, nextRelease}));
	}
